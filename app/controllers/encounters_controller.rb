...
=end
  def create
    ...
    health_passport = params[:concept]["Patient has Health Passport?"]
    service = params[:concept]["Service"]
    
    unless service.blank?
       service_id = BillingService.find_by_name(service).service_id
       unless service_id.blank?
         product = BillingProduct.find_by_service_id(service_id)
         unless product.blank?
           price = BillingPrice.find_by_product_id(product.product_id)
           raise price.to_yaml
         end
       end
    end
