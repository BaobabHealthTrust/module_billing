...
class CartItem
  attr_reader :product, :quantity, :price_type, :price
  ...
  def initialize(product,price_type = nil)
    ...
    @final_price = BillingPrice.find_by_product_id_and_price_type(@product.product_id,"General") if @final_price.blank?
  end
